...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "AntiBandera-ZELEN is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "AntiBandera-ZELEN is downloading blocks...");
...
UniValue checkkernel(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        ...
        if (vNodes.empty())
            throw JSONRPCError(-9, "AntiBandera-ZELEN is not connected!");

        if (IsInitialBlockDownload())
            throw JSONRPCError(-10, "AntiBandera-ZELEN is downloading blocks...");
